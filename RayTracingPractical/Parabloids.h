...
#ifndef RayTracingPractical_Parabloids_h
#define RayTracingPractical_Parabloids_h
class Parabloids : public Intersectable
{
    float4x4 A;
    float4x4 B;
public:
    Parabloids(Material* material)
    :Intersectable(material) {
        ...
        B = float4x4::identity;
        B._33 = -5;
        B = float4x4::translation(float3(0,0,2)) * B * float4x4::translation(float3(0,0,2)).transpose();
